...
RSpec.describe RecipeIngredient, type: :model do
  ...
  describe 'バリデーションのテスト' do
    it 'すべての属性が有効である場合、有効である' do
      recipe_ingredient = build(:recipe_ingredient, recipe:, ingredient: ingredient1)
      expect(recipe_ingredient).to be_valid
    end

    it 'recipeがない場合、無効である' do
      recipe_ingredient = build(:recipe_ingredient, recipe: nil, ingredient: ingredient1)
    ...
    it 'ingredientがない場合、無効である' do
      recipe_ingredient = build(:recipe_ingredient, recipe:, ingredient: nil)
      expect(recipe_ingredient).not_to be_valid
    ...
    it '同じrecipe_idとingredient_idの組み合わせは無効である' do
      ...
      duplicate_recipe_ingredient = build(:recipe_ingredient, recipe:, ingredient: ingredient1)
