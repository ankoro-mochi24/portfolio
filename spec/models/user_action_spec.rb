...
RSpec.describe UserAction, type: :model do
  ...
  describe 'バリデーションのテスト' do
    it 'すべての属性が有効である場合、有効である' do
      user_action = FactoryBot.build(:user_action, user:, actionable: recipe, action_type: 'good')
      expect(user_action).to be_valid
    end

    it 'action_typeがない場合、無効である' do
      user_action = FactoryBot.build(:user_action, user:, actionable: recipe, action_type: nil)
    ...
    it 'action_typeが無効な値の場合、無効である' do
      user_action = FactoryBot.build(:user_action, user:, actionable: recipe, action_type: 'invalid')
      expect(user_action).not_to be_valid
    ...
    it '同じユーザーが同じアイテムに対して同じaction_typeを持つ場合、無効である' do
      ...
      duplicate_user_action = FactoryBot.build(:user_action, user:, actionable: recipe, action_type: 'good')
    ...
    it '同じユーザーが同じアイテムに対してgoodとbadを同時に持つことはできない' do
      FactoryBot.create(:user_action, user:, actionable: recipe, action_type: 'good')
      conflicting_user_action = FactoryBot.build(:user_action, user:, actionable: recipe, action_type: 'bad')
  ...
  describe 'アソシエーションのテスト' do
    ...
    it 'actionableがRecipeモデルと関連付けられることを確認する' do
      user_action = FactoryBot.create(:user_action, user:, actionable: recipe, action_type: 'good')
      expect(user_action.actionable).to be_a(Recipe)
    end

    it 'actionableがFoodstuffモデルと関連付けられることを確認する' do
      user_action = FactoryBot.create(:user_action, user:, actionable: foodstuff, action_type: 'good')
      expect(user_action.actionable).to be_a(Foodstuff)
    end

    it 'actionableがToppingモデルと関連付けられることを確認する' do
      user_action = FactoryBot.create(:user_action, user:, actionable: topping, action_type: 'good')
