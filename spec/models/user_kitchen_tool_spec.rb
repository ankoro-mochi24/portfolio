...
RSpec.describe UserKitchenTool, type: :model do
  ...
  describe 'バリデーションのテスト' do
    it 'すべての属性が有効である場合、有効である' do
      user_kitchen_tool = build(:user_kitchen_tool, user:, kitchen_tool:, kitchen_tool_name: kitchen_tool.name)
    ...
    it 'userとkitchen_toolの組み合わせが一意でない場合、無効である' do
      create(:user_kitchen_tool, user:, kitchen_tool:, kitchen_tool_name: kitchen_tool.name)
      duplicate_user_kitchen_tool = build(:user_kitchen_tool, user:, kitchen_tool:, kitchen_tool_name: kitchen_tool.name)
  ...
  describe 'アソシエーションのテスト' do
    it 'ユーザーと調理器具に関連している' do
      user_kitchen_tool = build(:user_kitchen_tool, user:, kitchen_tool:)
