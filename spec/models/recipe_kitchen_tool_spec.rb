...
RSpec.describe RecipeKitchenTool, type: :model do
  ...
  describe 'バリデーションのテスト' do
    it 'すべての属性が有効である場合、有効である' do
      recipe_kitchen_tool = FactoryBot.build(:recipe_kitchen_tool, recipe:, kitchen_tool:, kitchen_tool_name: kitchen_tool.name)
      expect(recipe_kitchen_tool).to be_valid
    end

    it 'kitchen_tool_nameがない場合、無効である' do
      recipe_kitchen_tool = FactoryBot.build(:recipe_kitchen_tool, recipe:, kitchen_tool_name: nil)
  ...
  describe 'アソシエーションのテスト' do
    it 'レシピと調理器具に関連している' do
      recipe_kitchen_tool = FactoryBot.build(:recipe_kitchen_tool, recipe:, kitchen_tool:)
  ...
  describe '重複チェックのテスト' do
    it '同じrecipe_idとkitchen_tool_idの組み合わせは無効である' do
      ...
      duplicate_recipe_kitchen_tool = FactoryBot.build(:recipe_kitchen_tool, recipe:, kitchen_tool:)
